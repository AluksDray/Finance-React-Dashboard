import { useState } from "react";
import ExpensesDetails from "./expenses component/ExpensesDetails";
import ChatAndSearchComp from "../Income Folder/Income segment components/SearchInputComp";
import { useLoaderData } from "react-router-dom";

export default function CryptoExpenses(){


    const cryptoExpensesLoader = useLoaderData()

    const totalCryptoExpenses = cryptoExpensesLoader.reduce(
        (sum, item)=>sum + parseFloat(item.Amount.replace(/[^0-9.-]+/g, "")),0);

        const cryptoOptions = [
            { value: "BTC", label: "BTC" },
            { value: "ETH", label: "ETH" },
            { value: "SOL", label: "SOL" },
        ];

        const [selectCurrency, setSelectedCurrency] = useState("BTC")

        const conversionRates = {
            BTC: 1,
            ETH: 0.85,
            SOL: 0.67,
        };

        const convertedCrypto = (totalCryptoExpenses * conversionRates[selectCurrency])
            .toLocaleString(undefined, { style: "currency", currency: selectCurrency });
    return(
        <div>
            <ExpensesDetails 
            titleText="Total Crypto Expenses"
            amountText={convertedCrypto}
            expensesDrop={cryptoOptions}
            incomeData={cryptoExpensesLoader}
            expensesDropChange={(e) => setSelectedCurrency(e.target.value)}
            />

            <ChatAndSearchComp 
            inputholder="Talk to Our Al Finance expert"
            />
        </div>
    )
}